...
class SpiralGrid:
    ...
    def load(self, filename: str) -> bool:
        if path.exists(filename):
            ...
            print(f"File '{filename}' does not exist. Grid remains unchanged.")
            return False


def test_unlimited() -> None:
    grid = SpiralGrid()
    grid.load("grid_ul.pkl")
    ...
    print(bounds)
    grid.save("grid_ul.pkl")


def test_limited() -> None:
    grid = SpiralGrid((20, 20))
    grid.load("grid_l.pkl")
